...
def zoom_whoami() -> str:
    """
    Diagnostic: resolve the configured ZOOM_USER_ID to an actual Zoom user
    and report their Cloud Recording settings.

    Use this whenever list_recent_recordings/get_meeting_transcript come
    back empty despite meetings clearly having happened -- it answers the
    two most common causes directly instead of guessing:
      1. ZOOM_USER_ID pointing at the wrong user (compare "resolved_user"
         below against who you expected).
      2. Cloud Recording (or its audio-transcript sub-setting) simply being
         turned off for that user -- if so, Zoom never uploads anything to
         the cloud regardless of how many meetings happen, and there's
         nothing for this connector to fetch no matter how it's written.
         AI Companion meeting summaries (list_recent_meeting_summaries) are
         a SEPARATE feature from Cloud Recording and can keep working fine
         even while this is off, which is a common source of confusion.
    ...
    return _ok({
        ...
        "read_this_as": (
            ...
            "enable Cloud Recording (and record_audio_transcript specifically, "
            "which is a separate sub-toggle) in Zoom's web portal under "
    ...
    if port:
        ...
        if not AUTH_TOKEN:
            raise RuntimeError(
                ...
                "real meeting recordings/transcripts, and running unauthenticated "
                "would leave it open to anyone with the URL. Set MCP_AUTH_TOKEN and "
                "redeploy."
            )
